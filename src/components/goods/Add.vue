...
export default {
  ...
  methods: {
    ...
    handleRemove(file) {
      // 1.获取将要删除的图片的临时路径
      const fliePath = file.response.data.tem_path;
    ...
    handleSuccess(response) {
      ...
      // 2.将图片信息对象，push到pics数组里
      this.addForm.pics.push(picInfo);
      console.log(this.addForm);
    },
    foreachFilter(arr, type) {
      //   console.log(type == true);
      arr.forEach((item) => {
        let newInfo = {
          attr_id: item.attr_id,
          attr_value: type == 1 ? item.attr_vals.join(" ") : item.attr_vals,
        };
    ...
    add() {
      this.$refs.addFormRef.validate((valid) => {
        ...
        this.foreachFilter(this.manyTabData, 1);
        //   处理静态属性;
        ...
        //   });
        //   console.log(this.onlyTabData, "******");
        this.foreachFilter(this.onlyTabData, -1);
